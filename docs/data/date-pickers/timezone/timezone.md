...
Only `AdapterDayjs`, `AdapterLuxon`, `AdapterMoment` and `AdapterTemporalZonedDateTime` are currently compatible with UTC dates and timezones.
:::
...
function App() {
  return (
    <LocalizationProvider dateAdapter={AdapterTemporalZonedDateTime}>
      <DateTimePicker
...
To configure the zone used by the `"default"` timezone, call `setDefaultTimezone` exported from `@mui/x-date-pickers/AdapterTemporalZonedDateTime`:

```tsx
import { setDefaultTimezone } from '@mui/x-date-pickers/AdapterTemporalZonedDateTime';
...
| -----------------------------: | :----------------------- | :------------------------------------ |
| `AdapterTemporalZonedDateTime` | `Temporal.ZonedDateTime` | An exact point in time                |
...
The `timezone` prop and `setDefaultTimezone` only apply to `AdapterTemporalZonedDateTime`.
:::
