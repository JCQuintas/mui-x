...
The Temporal adapters use the standard [Temporal](https://tc39.es/proposal-temporal/docs/) API instead of a third-party date library.
There is one adapter per Temporal type, so the value keeps the meaning it has in your application:

|                        Adapter | Value type               |
| -----------------------------: | :----------------------- |
| `AdapterTemporalZonedDateTime` | `Temporal.ZonedDateTime` |
|     `AdapterTemporalPlainDate` | `Temporal.PlainDate`     |
|     `AdapterTemporalPlainTime` | `Temporal.PlainTime`     |
| `AdapterTemporalPlainDateTime` | `Temporal.PlainDateTime` |

See [Working with plain types](/x/react-date-pickers/timezone/#working-with-plain-types) for how to choose between them.

:::warning
`Temporal` is not yet available in every environment.
The adapters read the global `Temporal` object but do not bundle a polyfill, so you must provide one until your runtime supports `Temporal` natively.
...
They throw an error at instantiation if the global `Temporal` object is missing.
:::
...
The Temporal adapters use [BCP 47 language tags](https://developer.mozilla.org/en-US/docs/Web/JavaScript/Reference/Global_Objects/Intl/Locale) (for example `de-DE`), resolved through the native `Intl` API.
...
The Temporal adapters share their own set of tokens, since `Temporal` has no format string of its own: `yyyy`/`yy` (year), `MMMM`/`MMM`/`MM`/`M` (month), `dd`/`d` (day), `EEEE`/`EEE`/`EE` (weekday), `HH`/`H` (24-hour), `hh`/`h` (12-hour), `mm`/`m` (minute), `ss`/`s` (second), `SSS` (millisecond), and `a` (meridiem).
...
The Temporal adapters read the start of the week from the locale through the [browser API](https://developer.mozilla.org/en-US/docs/Web/JavaScript/Reference/Global_Objects/Intl/Locale/getWeekInfo).
